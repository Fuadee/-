...
test("schema ใหม่มี field ครบ: map ได้ครบทุกค่า", () => {
  ...
  assert.equal(result.summary.value, "ซื้อวัสดุสำนักงาน - เพื่อใช้งานในแผนกบัญชี");
...
test("schema ใหม่ขาดบาง field แต่ fallback ยังใช้งานได้", () => {
  ...
  assert.equal(result.summary.value, "จ้างบริการบำรุงรักษา - ซ่อมแซมระบบไฟฟ้า");
